...
app.post('/login', loginLimiter, async (req, res) => {
    ...
    try {
        ...
        const userQueryResponse = await callSankhyaService("DbExplorerSP.executeQuery", { sql: `SELECT CODUSU FROM TSIUSU WHERE NOMEUSU = '${sanitizeStringForSql(username.toUpperCase())}'` });
        ...
        if (clientDeviceToken) {
            const deviceCheckResponse = await callSankhyaService("DbExplorerSP.executeQuery", { sql: `SELECT ATIVO FROM AD_DISPAUT WHERE CODUSU = ${codUsu} AND DEVICETOKEN = '${sanitizeStringForSql(clientDeviceToken)}'` });
    ...
    try {
        ...
        if (filtro) {
            ...
            if (/^\d+$/.test(filtroLimpo)) {
                ...
                sqlFinal += ` AND (ENDE.SEQEND LIKE '${sanitizeStringForSql(filtroLimpo)}%' OR ENDE.CODPROD = ${filtroNum} OR ENDE.CODPROD = (SELECT CODPROD FROM AD_CADEND WHERE SEQEND = ${filtroNum} AND CODARM = ${codArm} AND ROWNUM = 1))`;
...
apiRoutes.post('/get-item-details', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Ocorreu um erro ao buscar os detalhes do item.' });
    }
});
...
apiRoutes.post('/get-picking-locations', async (req, res) => {
    try {
        ...
        res.status(500).json({ message: 'Ocorreu um erro ao buscar locais de picking.' });
    }
});
...
apiRoutes.post('/get-history', async (req, res) => {
    ...
    try {
        ...
        res.status(500).json({ message: 'Ocorreu um erro ao buscar seu histórico.' });
    }
});
    ...
    try {
        ...
        if (type === 'baixa') {
            ...
            recordsToSave.push({
                ...
                values: { "2": sanitizedPayload.codarm.toString(), "3": sanitizedPayload.sequencia.toString(), "4": sanitizedPayload.quantidade.toString() }
            });
            ...
            if (destinationItem && destinationItem[0] === sanCodProd) {
                recordsToSave.push({
                    ...
                    values: { "2": sanArmazemDestino.toString(), "3": sanEnderecoDestino, "4": destinationItem[1].toString() }
                });
            }
            recordsToSave.push({
                entityName: "AD_IBXEND", fields: ["SEQITE", "SEQBAI", "CODARM", "SEQEND", "ARMDES", "ENDDES", "QTDPRO"],
                values: { "2": sanCodArm.toString(), "3": sanSequencia.toString(), "4": sanArmazemDestino.toString(), "5": sanEnderecoDestino, "6": sanQuantidade.toString() }
            });
        }
        ...
        res.status(500).json({ message: 'Ocorreu um erro ao executar a transação.' });
    }
});
